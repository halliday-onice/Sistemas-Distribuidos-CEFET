...
public class BalanceadorUm {
	...
	private static int servQuantity = 3;
			...
			try {
				...
				while ((message = reader.readLine()) != null) {
					...
					if (message != null) {
						...
						if (message.equals("-2") || message.equals("-1") || message.equals("0")) {
							...
							String flagServ = message; // Recebe qual servidor está ativo ou inativo
								...
								BalanceadorUm.serv.add(servPort);
	
								BalanceadorUm.servQuantity += 1;
					...
					if (!listA.isEmpty()) {
						...
						Execution thread = new Execution(strNum, this.semaphore, BalanceadorUm.serv, port);						
						Thread t1 = new Thread(thread);
						t1.start();
						...
						Execution thread = new Execution(strNum, this.semaphore, BalanceadorUm.serv, flag);						
						Thread t1 = new Thread(thread);
						t1.start();
			...
			try {
				...
				if (servPort == "0") {
					...
					semaphore.acquire();
				     ...
				     int randomNumber = random.nextInt(BalanceadorUm.servQuantity);
				     ...
				     try {
						 ...
						 acc.close();
				        	...
				        	BalanceadorUm.serv.remove(Integer.valueOf(port));
				        	String nameFile = "file" + port+ ".txt";
				        	
				        	BalanceadorUm.dicionario.put(port, nameFile);
				     ...
				     if (connected == false) {
				    	 ...
				    	 randomNumber = random.nextInt(BalanceadorUm.servQuantity);
					     
					     port = BalanceadorUm.serv.get(randomNumber);
					...
					if (servPort.equals("4001")) {
						...
						List<Integer> copia = new ArrayList<>(BalanceadorUm.serv);
					...
					//Se o meu dicionário estiver com algum elemento, quer dizer que precisamos atualizar o seu arquivo correspondente
					
					if (!BalanceadorUm.dicionario.isEmpty()) {
